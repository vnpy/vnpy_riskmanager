...
本阶段的目标是解决底层引擎依赖过重的问题，并为性能敏感的场景引入Cython优化方案，提升系统的解耦性和运行效率。

- [x] **任务 5.1: 重构 `RuleTemplate` 与 `RiskEngine` 的交互机制**
    - **目标**: 解除 `RuleTemplate` 对 `MainEngine` 和 `EventEngine` 的直接依赖，转由 `RiskEngine` 统一管理事件订阅和数据分发，实现按需推送。
    - **步骤**:
        - 移除 `RuleTemplate` 构造函数中的 `main_engine` 和 `event_engine` 参数。
        - 在 `RiskEngine` 中，实现统一的事件监听接口（如 `on_tick`, `on_order`, `on_trade`），并主动调用规则实例中对应的同名方法。
        - `RiskEngine` 负责管理所有底层事件的订阅，并根据规则的需要，将数据精准分发给对应的规则实例。
        - 对于 `get_contract` 等数据查询需求，在 `RiskEngine` 中提供包装方法，由引擎代为向上层查询，避免规则直接接触 `MainEngine`。

- [x] **任务 5.2: 引入 Cython 对核心风控规则进行性能优化**
    - **目标**: 针对计算密集或高频触发的风控规则，使用 Cython 进行编译优化，降低交易延迟。
    - **步骤**:
        - 识别出性能瓶颈最高的风控规则（如滚动窗口计算类）。
        - 在项目中配置 `pyproject.toml`，增加 Cython 的编译构建支持。
        - 将选定的核心风控规则逻辑从 `.py` 文件迁移到 `.pyx` 文件中，并添加类型声明进行优化。
        - 编写基准测试，量化对比 Cython 化前后风控检查的性能提升。
        - 在文档中补充说明如何对新的风控规则进行 Cython 加速开发。

---

## Phase 6: 新增高级风控规则

本阶段将根据新的需求，开发一系列更精细、更复杂的风控规则，进一步增强风控矩阵的完整性。

- [x] **任务 6.1: 开发委托指令合法性监控规则**
    - **文件**: `vnpy_riskmanager/rules/order_validity_rule.py`
    - **目标**: 在委托发出前，检查其基础参数的合法性。
    - **步骤**:
        - 检查委托合约是否存在。
        - 检查委托价格是否为合约最小价格变动（`pricetick`）的整数倍。
        - 检查委托数量是否超过交易所或系统设置的单笔最大手数限制。

- [x] **任务 6.2: 开发重复报单监控规则**
    - **文件**: `vnpy_riskmanager/rules/duplicate_order_rule.py`
    - **目标**: 检测并拦截短时间内完全相同的重复委托。
    - **步骤**:
        - 监控同一合约在方向、开平、数量、价格上完全相同的委托请求。
        - 记录重复委托的次数，当超过设定的阈值时进行拦截并记录日志。

- [x] **任务 6.3: 开发全天委托/撤单笔数监控规则**
    - **文件**: `vnpy_riskmanager/rules/daily_limit_rule.py`
    - **目标**: 对整个交易日内的总委托和总撤单数量进行限制。
    - **步骤**:
        - 统计当日累计的委托笔数。
        - 统计当日累计的撤单笔数。
        - 超过设定的上限时，自动拦截后续请求。

- [x] **任务 6.4: 开发滚动窗口委托/撤单笔数监控规则**
    - **文件**: `vnpy_riskmanager/rules/rolling_window_rule.py`
    - **目标**: 对短时间内的委托和撤单频率进行限制。
    - **步骤**:
        - 使用 `deque` 结构，记录指定时间窗口（如1秒）内的所有委托和撤单请求的时间戳。
        - 实时计算窗口内的请求次数，超过频率限制时进行拦截。

- [x] **任务 6.5: 更新风控配置文件**
    - **文件**: `risk_engine_setting.json`
    - **目标**: 为所有新增的风控规则添加相应的控制参数。
    - **步骤**:
        - 添加例如 `max_duplicate_orders`, `daily_order_limit`, `rolling_window_seconds` 等新规则所需的配置字段。
